...
### 11.0 先搞清楚每条命令在哪台机器上执行

这是最容易搞混的地方：

| 步骤 | 在哪台机器 | 说明 |
|---|---|---|
| 11.1 `ssh root@IP` | **你的电脑** | 唯一在本地敲的命令，作用是登进服务器 |
| 11.2 之后全部 | **服务器** | 敲完 `ssh` 后，终端已经是服务器的了 |

判断依据很简单——看命令提示符：

```text
PS C:\Users\...>        ← 这是你的 Windows，还没登进服务器
root@iZbp1x2y3z:~#      ← 这是服务器，从这里开始的命令都在云端执行
```

所以**文档里所有 `sudo`、`apt`、`systemctl` 都是服务器上的命令**。你的 Windows 电脑不需要 sudo，也不需要开启任何相关设置。

另外，如果你用 `root` 登录服务器（阿里云默认），`sudo` 其实是多余的——root 本来就有全部权限。`deploy/install_service.sh` 会自动判断：是 root 就不用 sudo，不是 root 才用。
